...
export default function QuantitiesPage() {
  ...
  const scopedComparison = useMemo(() => (analysis?.baseline_changed_comparison || []).filter(row => dataScope === "전체" || originForText(...row.source_rows) === dataScope), [analysis, dataScope]);
  ...
  }, [analysis, dataScope, sourceSet, workPackage, resultCategory, mismatchReason, severity, query]);
  ...
  return <AppShell eyebrow="QUANTITY / FORMULA / COST" title="내역·수량 오류 분석">
    ...
    <section className="qa-set-summary panel">
      ...
      <div className="qa-set-grid"><button type="button" className={sourceSet === "전체" ? "selected" : ""} onClick={() => handleSourceSetChange("전체")}><span>전체</span><strong>{loading ? "—" : scopedItems.length}</strong><small>세트별 전체 검토</small></button><button type="button" className={sourceSet === "기준자료" ? "selected" : ""} onClick={() => handleSourceSetChange("기준자료")}><span>기준자료</span><strong>{loading ? "—" : scopedSummary["기준자료"] || 0}</strong><small>변경 전 자료 세트</small></button><button type="button" className={sourceSet === "변경자료" ? "selected" : ""} onClick={() => handleSourceSetChange("변경자료")}><span>변경자료</span><strong>{loading ? "—" : scopedSummary["변경자료"] || 0}</strong><small>변경 후 자료 세트</small></button><button type="button" className={sourceSet === "기준·변경 대조" ? "selected" : ""} onClick={() => handleSourceSetChange("기준·변경 대조")}><span>기준·변경 대조</span><strong>{loading ? "—" : scopedSummary["기준·변경 대조"] || 0}</strong><small>원천 행 연결 확인 필요</small></button></div>
    </section>
