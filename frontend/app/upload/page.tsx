...
export default function UploadPage() {
  ...
  return <AppShell eyebrow="00 / SOURCE INTAKE" title="자료 업로드·검증">
    ...
    <section className="panel"><div className="panel-head"><div><p className="eyebrow">SOURCE FILES</p><h2>등록된 원본 {loading ? "—" : `${files.length}건`}</h2></div><span className="review-only">경로·해시·추적정보</span></div>{loading ? <div className="review-loading">원본 파일 목록을 불러오는 중입니다…</div> : <div className="table-wrap upload-files-wrap"><table className="upload-files-table"><thead><tr><th>파일</th><th>적용 범위</th><th>문서·세트</th><th>도면 식별정보</th><th>상태</th><th>재시도</th></tr></thead><tbody>
